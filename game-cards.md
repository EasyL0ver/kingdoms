# Kingdoms — Card Catalogue

All designed cards in one place, organised by deck.  
*Auto-generated by `print_cards.py` — do not edit by hand.*

---

## Domain

```
🏰  PRESENCE
SETUP: Start with this card.
It cannot be discarded or moved.
On Dawn — choose one:
• Order on the Claw zone or Tree zone.
• Order a card in your Domain.
```

---

## Deck Zone Cards

One per deck. Placed next to their draw pile at setup.

```
🐾  CLAW ZONE
On Order — draw 2 cards from the Claw pile.
```

```
🌳  TREE ZONE
SETUP: 4 cards face-up — the Season.
On Order — take 1 card from the Season.
Refill the Season to 4.
```

```
🌾  WHEAT ZONE
SETUP: 5 face-up cards — the Village (conveyor).
On Order — take 1–5 from the bottom.
Reveal Claw cards equal to [Labour] tags
taken → to your Domain.
If [Discontent] in your Domain ≥ 3:
Revolt — Brawl in every Domain with Wheat.
Refills to 5 after drawing.
```

```
🪙  COIN ZONE
SETUP: 3 face-up cards — the Opportunities.
Discarded cards go to the Wares.
On Order — choose one:
• Buy — take 1 card from the Wares.
• Trade — give 1 Domain card to Wares,
  take 1 Opportunity. Rumour in every Domain.
```

```
🕯️  CANDLE ZONE
SETUP: 1 face-up card — the Revelation.
On Order — claim the Revelation.
Reveal the next card.
```

```
⚔️  SWORD ZONE
SETUP: 2 face-up cards — the Tourney.
On Order — Injustice (2+ [Mob] in any
Domain): tyrant takes [Unit], you take rest.
Peace (Joust): challenge an opponent.
Accept = both pick 1. Refuse = Brawl in
both Domains. Refill Tourney to 2.
```

---

## 🐾 Claw Deck Cards

> Violence, ambition, raw power, societal sentiment. The primal human element — anger, passion, desire, fear. The uncontrollable voice of the people that every civilisation must contend with.

```
🐾  WARBAND
[Discontent]
On Order — you may move 1 [Mob] to the Domain with the most cards. Brawl there.
```

```
🐾  RAID
[Unit] [Mob] [Discontent]
On Brawl — discard 1 card from your Domain. If the active player is someone else, they get the card instead.
```

```
🐾  SCAVENGE
[Unit] [Mob] [Discontent]
On Brawl — if the active player is someone else, they take 1 card from your discard.
```

```
🐾  BLOOD OFFERING
On Order — discard 1 card from your Domain. Rite in your Domain.
```

```
🐾  POACH
[Unit] [Mob] [Hunt] [Discontent]
On Order — Feast in your Domain. Draw 1 from Claw. Hunt — blocked if another player has [Hunt].
```

```
🐾  WORSHIP OF THE HUNT
[Spiritual]
On Rite — dump 5 Claw cards to the active player's discard.
```

```
🐾  WORSHIP OF THE DUSK
[Spiritual]
On Rite — every player discards 1 card from their Domain. If Worship of the Dawn is in play, discard 2 instead.
```

```
🐾  WORSHIP OF WAR
[Spiritual]
On Rite — the active player Brawls in a Domain of their choice.
```

```
🐾  INCITE
On Dawn — move up to 3 [Mob] cards from your Domain to other Domains. Discard Incite.
```

```
🐾  CHIEFDOM
[Trophy]
On Dawn — move up to 2 [Mob] cards from your Domain or a culture ally's to any other Domain.
```

```
🐾  KINSHIP
On Harvest — Order on the Tree zone.
```

```
🐾  RACKETEERING
[Discontent]
On Order — choose a player. They offer you 1 card. Take it, or refuse and Brawl in their Domain.
```

```
🐾  TYRANNY
[Trophy] [Discontent]
On Order — draw from Claw equal to your [Discontent] count. Brawl in your Domain.
```

```
🐾  MARAUDERS
[Unit] [Mob] [Discontent]
On Feast — discard Marauders. Draw 1 from Claw.
```

```
🐾  SHARE THE SPOILS
On Feast — draw 1 from Claw.
```

```
🐾  MARTIAL EXCELLENCE
[Trophy] [Chivalry]
On Order — Order on the Sword zone. Requires another [Trophy] in your Domain.
```

```
🐾  OUTRIDERS
On Order — draw 3 from Claw. Discard 1 of your choice.
```

```
🐾  LAND GRAB
[Discontent]
On Order — take all [Land] cards from the Season to your Domain. Discard Land Grab. Requires [Land] in the Season.
```

```
🐾  RITE OF PASSAGE
[Discontent]
On Brawl — draw 1 from Tree.
```

```
🐾  CULLING
[Discontent]
On Dawn — the player with the most cards discards 1–2 cards of their choice. Discard Culling.
```

```
🐾  IVORY
[Trophy] [Wealth]
On Order — discard Ivory. Order on the Coin zone.
```

```
🐾  HUNGER
[Discontent]
On Harvest — dump 1 Claw card to your discard. On Feast — return 1 card from your discard to top of the Claw pile.
```

```
🐾  UPRISING
[Discontent]
On Dawn — Brawl in your Domain. Discard Uprising.
```

```
🐾  RANSACK
On Order — discard 1 card from your Domain. Draw 2 from Claw. Take 1 from the Season.
```

```
🐾  SPOILS OF WAR
[Trophy] [Mob]
On Dawn — place Spoils of War in another player's Domain. On Brawl — moves to the active player. They draw from Claw and Tree equal to their [Trophy] count.
```

```
🐾  VEIL TEAR
[Spiritual] [Discontent]
On Order — purge any number of cards from your discard. Return that many [Spiritual] cards from any discard to their owner's Domain. Rite in every Domain that many times. Discard Veil Tear.
```

```
🐾  BLOOD FEUD
[Mob] [Discontent]
On Brawl — draw 2 from Claw. Move up to 2 [Mob] to the active player. Discard Blood Feud, then Brawl in the attacker's Domain.
```

```
🐾  ENFORCERS
[Mob] [Discontent]
On Brawl — both you and the active player draw 2 from Claw.
```

---

## 🌳 Tree Deck Cards

> Family, spirituality, harmony, roots, nature. Community bonds, ancestral wisdom, peaceful growth. The seasonal engine and communal heartbeat.

```
🌳  KINSHIP
On Harvest — Order on the Tree zone.
```

```
🌳  ELDERSHIP
[Knowledge]
On Brawl — if the active player shares your culture, you may cancel the Brawl. They draw 1 from Tree.
```

```
🌳  WORSHIP OF THE DAWN
[Spiritual]
On Rite — return 1 card from your discard to your Domain. If Worship of the Dusk is in play, return 2 instead.
```

```
🌳  HARVEST
On Dawn — Harvest in every zone. Discard Harvest.
```

```
🌳  GATHERING
On Dawn — choose Brawl, Rite, or Rumour. Fires in your Domain and all culture allies' Domains. Discard Gathering.
```

```
🌳  SACRED GROVE
[Nature] [Spiritual]
On Order — choose one: • Rite in your Domain. • Look at the top 3 Tree cards. Take any [Spiritual] cards. Return the rest.
```

```
🌳  WORSHIP OF THE HEARTH
[Nature] [Spiritual]
On Rite — Harvest in every Domain that has Kinship.
```

```
🌳  FORAGE
On Order — dump top 2 Tree and top 2 Claw cards to your discard. Feast in your Domain.
```

```
🌳  SOWING
[Knowledge]
On Order — Order on the Wheat zone. On Harvest — refill Season buffer, then take all [Nature] cards from Season for free. Requires 2+ [Nature] in your Domain.
```

```
🌳  COMPOST
On Order — purge a Harvest card from your discard. Harvest in your Domain.
```

```
🌳  VIGIL
On Harvest — for each player with Kinship, return 1 card from your discard to your Domain.
```

```
🌳  FLOODS
[Nature]
On Brawl — if another Floods is in play, cancel the Brawl. On Dawn — refill the Season. If another Floods is in play, every player discards 1 card from their Domain.
```

```
🌳  BOG
[Nature]
On Rumour — swallow 1 card from the Season.
```

```
🌳  BEAR'S DEN
[Nature]
On Feast — draw 1 from Claw.
```

```
🌳  RAVINE
[Nature]
On Brawl — the active player discards 1 card from their Domain.
```

```
🌳  MEADOW
[Nature]
On Harvest — refill the Season to full.
```

```
🌳  REGROWTH
On Dawn — all [Nature] cards in every player's discard return to their owner's Domain. Discard Regrowth.
```

```
🌳  HOSPITALITY
On Order — choose a player with Kinship. You may Order 1 card in their Domain. They may Order 1 card in yours.
```

---

## 🌾 Wheat Deck Cards

> Labour, agriculture, the common folk. The working class that sustains kingdoms — but every harvest comes with unrest.

```
🌾  ANIMAL HUSBANDRY
[Labour]
On Order — choose one: • Order on the Wheat zone. • Order on the Coin zone. • Feast in your Domain.
```

```
🌾  TAVERN
[Amenity]
On Feast — you may return 1 [Discontent] from your Domain to top of Claw pile. You may return 1 Claw card from your discard to top of Claw pile.
```

```
🌾  MILITIA
[Unit]
On Rumour — discard 1 [Mob] from your Domain. On Brawl — discard Militia to cancel the Brawl in your Domain.
```

```
🌾  CROOKED INN
[Amenity] [Discontent]
On Rumour — return 1 [Mob] from your discard to your domain.
```

```
🌾  ENCLOSURE
[Labour] [Amenity]
On Order — take any 1 card from the Village. Put 1 card from your domain back in its place.
```

```
🌾  TURNIP PATCH
[Labour]
On Order / On Harvest — Order on the Wheat zone.
```

```
🌾  RITUAL PYRE
[Nature] [Spiritual]
On Order — sacrifice a card from your domain. If you do, Harvest in your Domain. Rite in your Domain.
```

```
🌾  FOLK HERO
[Unit] [Trophy]
On Dawn — move 1 [Mob], [Labour], or [Amenity] card between domains that have Wheat cards.
```

```
🌾  WOLF PACK
[Mob] [Nature] [Discontent]
On Harvest — If you triggered it: move Wolf Pack to another player's domain. Otherwise: discard 2 from domain, draw 2 Claw.
```

```
🌾  HERB GARDEN
[Nature]
On Harvest — Order on the Tree zone or Wheat zone (your choice).
```

```
🌾  ORCHARD
[Nature] [Labour]
On Harvest — draw 1 from Tree. Feast in your Domain.
```

```
🌾  REEVE
[Unit]
On Dawn — order 1 Wheat card in your domain (fires its On Order effect).
```

```
🌾  WORSHIP OF THE BREAD
[Spiritual]
On Rite — rearrange the Village belt.
```

```
🌾  ORA ET LABORA
[Labour] [Spiritual]
On Harvest — Order on the Candle zone. On Rite — Order on the Wheat zone.
```

```
🌾  TAX COLLECTORS
[Mob] [Discontent]
On Order (requires 3+ [Labour]) — Order on the Coin zone. Rumour in your Domain.
```

```
🌾  LOOKOUT
On Dawn — if you have [Discontent], Rumour in your domain.
```

```
🌾  ILL TIDINGS
On Rumour — choose one, then discard this: • Panic — draw 2 from Claw. • Fortify — return 1 [Discontent] to the Claw pile.
```

```
🌾  PILGRIMAGE
[Spiritual]
On Order — give 1+ cards from your domain to another player. They decide: if they accept, draw that many Candle cards. If they refuse, no reward. Discard this.
```

---

## 🪙 Coin Deck Cards

> Trade, commerce, social mobility. The merchant class and flow of wealth. The Wares evolve into a chaotic bazaar as players trade cards through it.

```
🪙  TREASURE
[Trophy] [Amenity] [Wealth]
No effect — pure tags.
```

```
🪙  MARKET
On Order — offer a Ware to a player (buyer picks which). If they accept, Order on the Coin zone.
```

```
🪙  PAWN SHOP
On Order — Order on the Coin zone. On Rumour — swap 1 Domain card with 1 Wares card.
```

```
🪙  STOCKPILE
On Order — put 1 Domain card into Wares. Order on the Coin zone.
```

```
🪙  SMUGGLER
[Mob]
On Brawl — controller picks 1 from Wares, Brawl starter takes it. (No Opp. refill.) Smuggler moves to starter's domain.
```

```
🪙  FORGERY
[Discontent]
On Order — if you have a Coin card in Domain, put Forgery into Wares. Order on the Coin zone.
```

```
🪙  USURER
On Order — put 1 Domain card into Wares. On Rumour — Order on the Coin zone.
```

```
🪙  HIGHWAYMEN
[Mob]
On Brawl — put 1 Domain card into Wares. Brawl starter Orders on the Coin zone.
```

```
🪙  SELLSWORD
[Unit]
On Brawl — discard Sellsword to the Wares to cancel the Brawl in your Domain.
```

```
🪙  SWINDLE
On Order — give all Wares to a chosen opponent. Brawl in their Domain. Discard Swindle.
```

```
🪙  PROSPERITY
[Wealth]
On Dawn — draw 1 from Coin if Opportunities exist.
```

```
🪙  EMBASSY
[Wealth]
On Dawn — if you have a culture ally and Wares exist: you and your ally each take 1 card from the Wares.
```

```
🪙  EFFICIENCY
On Order — Order up to 4 other cards in your Domain. Discard Efficiency.
```

```
🪙  SPICE MARKET
[Wealth]
On Order — draw Coin cards equal to your count of unique tags in your Domain.
```

```
🪙  COMMODITIES
On Rumour — draw 1 card from a chosen pile. Add it to the Wares.
```

```
🪙  PROVISIONS
[Amenity]
On Feast — draw 1 from Coin.
```

```
🪙  WORSHIP OF GOLD
[Spiritual]
On Rite — the active player takes 1 card from the Wares.
```

---

## 🕯️ Candle Deck Cards

> Faith, religion, moral authority, divine right. Alternative path to power through spiritual influence.

```
🕯️  WORSHIP OF THE SCRIPTURE
[Spiritual] [Religion]
On Rite — the active player peeks at Candle cards (scales with [Spiritual]). Keep 1, purge the rest.
```

```
🕯️  WORSHIP OF THE RELIC
[Spiritual] [Religion]
On Rite — peek at cards from a chosen pile (scales with [Spiritual]). May replace the Revelation. Return rest to top.
```

```
🕯️  WORSHIP OF THE MARTYR
[Spiritual] [Religion]
On Rite — the active player may discard cards. All other players must discard equal to [Spiritual] scaling.
```

```
🕯️  CLERGY
[Religion]
On Order — Order on the Candle zone. Peek at Candle cards (1 per player with [Religion]). Keep 1, may set 1 as Revelation, purge rest.
```

```
🕯️  SABBATH
[Religion]
On Dawn — Rite in your Domain.
```

```
🕯️  ZEALOT
[Spiritual] [Religion] [Mob]
On Brawl — cancel if you have [Religion]; else discard 1 Domain card. On Rite — active player may move Zealot to another Domain.
```

```
🕯️  ALMS
[Religion]
On Feast — refill 1 Field. Return 1 [Discontent] to the Claw pile.
```

```
🕯️  EVANGELISM
[Religion]
On Order — each player in turn order claims 1 Revelation. Refill between each.
```

```
🕯️  PURITY
[Religion]
On Order — optionally purge the Revelation and reveal the next. Rite in your Domain.
```

```
🕯️  FLAGELLATION
[Religion]
On Rite — Brawl in your Domain (no player benefits).
```

```
🕯️  PENANCE
[Spiritual] [Religion]
On Dawn — discard 2 cards from your Domain. You may sacrifice Penance for an extra effect.
```

```
🕯️  BENEFACTION
[Religion]
On Order — draw 1 from Coin. Refill Opportunities to 3. Rumour in your Domain twice.
```

```
🕯️  ORNAMENT
[Religion]
On Order — move the Revelation to the Wares. Reveal the next card.
```

```
🕯️  PROTECT THE MEEK
[Chivalry]
On Brawl — Order on the Sword zone.
```

---

## ⚔️ Sword Deck Cards

> Chivalry, martial prowess, feudal loyalty. The warrior aristocracy and feudal military order.

```
⚔️  ROYAL HUNT
[Unit] [Trophy] [Hunt]
On Order — Feast in your Domain twice. Draw 1 from Claw. Hunt — blocked if another player has [Hunt].
```

```
⚔️  KNIGHT OF THE GOLD COAT
[Unit] [Chivalry] [Wealth]
On Rumour / On Brawl — slay a [Unit] anywhere. Knight moves to that Domain.
```

```
⚔️  KNIGHT OF THE IRON CROWN
[Unit] [Chivalry] [Trophy]
On Dawn — slay a [Unit] anywhere. Knight moves to that Domain.
```

```
⚔️  KNIGHT OF THE GREEN MANTLE
[Unit] [Chivalry] [Nature]
On Harvest — slay a [Unit] anywhere. Knight moves to that Domain.
```

```
⚔️  KNIGHT OF THE WHITE SHIELD
[Unit] [Chivalry] [Amenity]
On Feast — slay a [Unit] anywhere. Knight moves to that Domain.
```

```
⚔️  KNIGHT OF THE HOLY CROSS
[Unit] [Chivalry] [Religion]
On Rite — slay a [Unit] anywhere. Knight moves to that Domain.
```
